...
#             reward+=(info["x_pos"]-self.x_prev)*0.1     
#         if info["x_pos"]>self.far_x:
#             reward+=(info["x_pos"]-self.far_x)
#             # reward+=abs(info["y_pos"]-self.y_prev)*1.5
#             self.far_x = info["x_pos"]
#             self.no_progress=0
#         if self.no_progress>32:
#             if info["x_pos"]<self.x_prev: 
#                 reward-=abs(info["x_pos"]-self.x_prev)*0.5
...
def train(opt,writer):
    ...
    memory_entropy = torch.zeros(opt.memory_size, 1, dtype=torch.float32)
    ...
    if torch.cuda.is_available():
        ...
        memory_entropy = memory_entropy.cuda()
    else:
    ...
    for iter in range(opt.num_global_steps):
        ...
        while True:
            ...
            if torch.cuda.is_available():
                ...
                if steps>opt.burn_in:
                    memory_entropy[steps%opt.memory_size]=old_m.entropy().detach()            
                else:
                    memory_entropy[steps%opt.memory_size]= torch.log(torch.tensor(len(LEFTRIGHT_MOVEMENT)))
            else:
                memory_s[steps%opt.memory_size]=state
                memory_s_info[steps%opt.memory_size]=state_info.clone()
                memory_a[steps%opt.memory_size]=action
        ...
        if steps>opt.burn_in:
            ...
            if curr_episode%opt.update_freq==0:
                for _ in range(opt.gradient_steps):
                    ...
                    with torch.no_grad():
                        q1_all_target = model_q1_target((states,p_state_infos))               
                        q2_all_target = model_q2_target((states,p_state_infos))
                    ...
                    q1_target = q1_all_target.gather(1,actions).detach()                  
                    q2_target = q2_all_target.gather(1,actions).detach()

                    mean_q1 = q1.detach().mean().item()
                    mean_q2 = q2.detach().mean().item()

                    clip_q1 = q1_target +torch.clamp(q1-q1_target,-opt.clip_value,opt.clip_value)
                    clip_q2 = q2_target +torch.clamp(q2-q2_target,-opt.clip_value,opt.clip_value)
                    q1_loss = torch.max(torch.mean((q1-target_q)**2)/2,torch.mean((clip_q1-target_q)**2)/2)
                    q2_loss = torch.max(torch.mean((q2-target_q)**2)/2,torch.mean((clip_q2-target_q)**2)/2)
                    ...
                    with torch.no_grad():
                        q1_all = model_q1((states,p_state_infos))
                        q2_all = model_q2((states,p_state_infos))
                    H_mean = -torch.sum(probs*log_probs,dim=1)
                    

                    avg_q_all= (q1_all+q2_all)/2
                    a_loss = torch.sum(probs * (alpha*log_probs - avg_q_all), dim=1, keepdim=True).mean()+0.25*((entropy-H_mean)**2).mean()
        ...
        if curr_episode % opt.update_freq == 0: 
            ...
            if temp_reward_sum>=max_reward:
                ...
                torch.save(model_actor.state_dict(),"best_sac_actor_nstep_t.pt")  
                torch.save(model_q1.state_dict(),"best_sac_q1_nstep_t.pt")  
                torch.save(model_q1_target.state_dict(),"best_sac_q1_target_nstep_t.pt")  
                torch.save(model_q2.state_dict(),"best_sac_q2_nstep_t.pt")  
                torch.save(model_q2_target.state_dict(),"best_sac_q2_target_nstep_t.pt")  
...
def get_args():
    ...
    parser.add_argument("--saved_path", type=str, default="trained_models/sac_t")
