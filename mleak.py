...
class CustomReward(gym.RewardWrapper):
    ...
    def step(self, action):
        ...
        if info["coins"]>self.curr_coin:
            reward += (info["coins"]-self.curr_coin)*1200*(1+info["x_pos"]/4800)
            if info["score"] > (info["coins"]-self.curr_coin)*200+self.curr_score:
                reward += ((info["score"] - self.curr_score)-(info["coins"]-self.curr_coin)*200)*6*(1+info["x_pos"]/4800)
            self.no_progress=0            
        elif (info["score"] - self.curr_score)>0:
            reward += (info["score"] - self.curr_score)*6*(1+info["x_pos"]/4800)
            self.no_progress=0
...
class Resetstate(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if done or self.stuck>64 or info_1[1]>256 or info_1[2]:            
            ...
            if self.stuck>64:
                ...
                # self.y_pos = 163
                # self.y_pre_pos=163
                return obs, reward-200/400, True, (info_1[0],info_1[1],65,False), info
            elif info_1[1]>256:
                self.stuck=0
                self.x_pos = 40
                self.x_pre_pos=40
                # self.y_pos = 163
                # self.y_pre_pos=163
                return obs, reward-500/400, True, (info_1[0],info_1[1],self.stuck,False), info
            elif info_1[2]:
                self.stuck=0
                self.x_pos = 40
                self.x_pre_pos=40
                # self.y_pos = 163
                # self.y_pre_pos=163
                return obs, reward-400/400, True, (info_1[0],info_1[1],self.stuck,True), info
            else:
                self.stuck=0
                self.x_pos = 40
                self.x_pre_pos=40
                # self.y_pos = 163
                # self.y_pre_pos=163
                return obs, reward-100/400, done, (info_1[0],info_1[1],self.stuck,False), info
...
class SAC_softQ(nn.Module):
  ...
  """
    ...
    def __init__(self, input_dim, output_dim):
        ...
        self.critic_linear = nn.Linear(512, output_dim)
        self._initialize_weights()
    def _initialize_weights(self):
        for module in self.modules():
            if isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear):
...
def train(opt,writer):
    ...
    model_actor = SAC_actor((env.observation_space.shape,10),len(COMPLEX_MOVEMENT))
    model_q1 = SAC_softQ((env.observation_space.shape,10),len(COMPLEX_MOVEMENT))
    model_q1_target = SAC_softQ((env.observation_space.shape,10),len(COMPLEX_MOVEMENT))
    model_q2 = SAC_softQ((env.observation_space.shape,10),len(COMPLEX_MOVEMENT))
    model_q2_target = SAC_softQ((env.observation_space.shape,10),len(COMPLEX_MOVEMENT))
    memory = []
    alpha=torch.tensor(opt.alpha, dtype=float, requires_grad=False)
    
    target_entropy = torch.tensor(opt.alpha_min * (-np.log(1 / len(COMPLEX_MOVEMENT))),requires_grad=False)

    
    if torch.cuda.is_available():
        model_actor.cuda()
        model_q1.cuda()
        model_q1_target.cuda()
        model_q2.cuda()
        model_q2_target.cuda()
        alpha=alpha.cuda()
    ...
    for iter in range(opt.num_global_steps):
        ...
        if curr_episode % opt.save_interval == 0 and curr_episode > 0:
            torch.save(model_actor.state_dict(),"{}/ppo_super_mario_bros_{}_{}_{}_SAC_actor".format(opt.saved_path, opt.world, opt.stage, curr_episode))
            torch.save(model_q1.state_dict(),"{}/ppo_super_mario_bros_{}_{}_{}_SAC_q1".format(opt.saved_path, opt.world, opt.stage, curr_episode))
            torch.save(model_q1_target.state_dict(),"{}/ppo_super_mario_bros_{}_{}_{}_SAC_q1_target".format(opt.saved_path, opt.world, opt.stage, curr_episode))
            torch.save(model_q2.state_dict(),"{}/ppo_super_mario_bros_{}_{}_{}_SAC_q2".format(opt.saved_path, opt.world, opt.stage, curr_episode))
            torch.save(model_q2_target.state_dict(),"{}/ppo_super_mario_bros_{}_{}_{}_SAC_q2_target".format(opt.saved_path, opt.world, opt.stage, curr_episode))
        state_info = torch.zeros(1,10)
        state_info[0][0]=40/1280
        state_info[0][1]=163/1024
        state_info[0][2]=40/1280                
        state_info[0][3]=163/1024
        ...
        if torch.cuda.is_available():
            ...
            state=state.cuda()
            p_state_info=p_state_info.cuda()
        while True:
            if steps>opt.burn_in:
                ...
                action =torch.randint(1,len(COMPLEX_MOVEMENT),(1,))
            ...
            if torch.cuda.is_available():
                ...
                # print(state.device)
                print(f"{len(memory)}!!!!!")
                if len(memory)<opt.memory_size:
                    memory.append([state.to("cuda:0"),next_state.to("cuda:0")])
                else:
                    memory[steps%opt.memory_size]=[state.cuda()]
                
            else:
                memory.append([state,next_state,action.reshape(1).detach(),torch.tensor([reward]).float(),p_state_info,state_info.clone(),torch.tensor([done]).float()])
            p_state_info = state_info.clone()
            if torch.cuda.is_available():
                p_state_info=p_state_info.cuda()      
            state=next_state
        ...
        #             ind = np.random.randint(len(memory),size=opt.batch_size)
        #             samples = [memory[index] for index in ind]
                   
                        
        #             states, next_states, actions, rewards,p_state_infos,n_state_infos,dones = zip(*samples) 
        ...
        #             # print(rewards.shape)
        #             p_state_infos=torch.cat(p_state_infos)
        #             # print(p_state_infos.shape)
        #             n_state_infos=torch.cat(n_state_infos)
        #             # print(n_state_infos.shape)
        ...
        #                 # print(f"next probs {next_probs}",file=f)
        #                 # print(f"next log probs {next_log_probs}",file=f)
        #                 next_q1 = model_q1_target((next_states,n_state_infos))
        #                 next_q2 = model_q2_target((next_states,n_state_infos))
        #                 # print(f"next q1 {next_q1}",file=f)
        #                 # print(f"next q2 {next_q2}",file=f)
        #                 next_q = torch.min(next_q1,next_q2)
        #                 v_next = torch.sum(next_probs*(next_q-alpha*next_log_probs),dim=1, keepdim=True) 
        #                 # print(f"next v {v_next}",file=f)    
        #                 target_q= rewards.reshape(-1,1)+(1-dones).reshape(-1,1)*opt.gamma*v_next
        #                 # print(f"target q {target_q}",file=f) 
        #             q1_all = model_q1((states,p_state_infos))
               
        #             q2_all = model_q2((states,p_state_infos))
        ...
        #             mean_q2 = q2.detach().mean().item()
        #             q1_loss = torch.mean((q1-target_q)**2)/2
        #             q2_loss = torch.mean((q2-target_q)**2)/2
        #             q_loss = q1_loss+q2_loss
        #             # print(f"q_loss: {q_loss}",file=f)
        #             optimizer_q1.zero_grad()
        #             optimizer_q2.zero_grad()
        #             q_loss.backward()
        #             # torch.nn.utils.clip_grad_norm_(model_q1.parameters(), 0.5)
        #             # torch.nn.utils.clip_grad_norm_(model_q2.parameters(), 0.5)
        #             optimizer_q1.step()
        #             optimizer_q2.step()                        
        ...
        #             # print(f"probs {probs}",file=f)
        #             # print(f"log probs {log_probs}",file=f)
                 
        #             with torch.no_grad():
        #                 q1_all = model_q1((states,p_state_infos))
        #                 q2_all = model_q2((states,p_state_infos))

        #                 # print(f"actor q1 {q1_all}",file=f)
        #                 # print(f"actor q2 {q2_all}",file=f)


        #             min_q_all= torch.min(q1_all,q2_all)
        #             a_loss = torch.sum(probs * (alpha*log_probs - min_q_all), dim=1, keepdim=True).mean()
        ...
        #             # update alpha
        #             with torch.no_grad():
        #                 H_mean = torch.sum(probs*log_probs,dim=1).mean()
        ...
        #         print("Episode: {} steps: {} q1: {} q1 loss: {} q2 {} q2 loss: {} actor loss: {} alpha loss: {} alpha:{} entropy:{}".format(curr_episode,steps,mean_q1,q1_loss,mean_q2,q2_loss,a_loss,alpha_loss,alpha,-H_mean))
        #         writer.add_scalar('q1_Loss',q1_loss, curr_episode)
        #         writer.add_scalar('q2_Loss',q2_loss, curr_episode)
        ...
        #     success_=False
        #     stuck=0
        #     state = torch.from_numpy(np.array(env_eval.reset(seed=114514,options={})[0]).reshape(1,4,84,84))
        #     r_state_info = torch.zeros(1,10)
        #     r_state_info[0][0]=40/1280
        #     r_state_info[0][1]=163/1024
        #     r_state_info[0][2]=40/1280                
        #     r_state_info[0][3]=163/1024
        ...
        #     p_r_state_info=r_state_info.clone()
        #     if torch.cuda.is_available():
        #         r_state_info=r_state_info.cuda()
        #         state=state.cuda()
        #         p_r_state_info=p_r_state_info.cuda()
        #     # if record==1:
        ...
        #             memory.append([state,next_state,action.reshape(1),torch.tensor([reward]).float().cuda(),p_r_state_info,r_state_info.clone(),torch.tensor([done]).float().cuda()])
        #         else:
        #             memory.append([state,next_state,action.reshape(1),torch.tensor([reward]).float(),p_r_state_info,r_state_info.clone(),torch.tensor([done]).float()])
        #         p_r_state_info = state_info.clone().detach()
        #         if torch.cuda.is_available():
        #             p_r_state_info=p_r_state_info.cuda()  
        ...
        #         torch.save(model_actor.state_dict(),"best_sac_actor.pt")  
        #         torch.save(model_q1.state_dict(),"best_sac_q1.pt")  
        #         torch.save(model_q1_target.state_dict(),"best_sac_q1_target.pt")  
        #         torch.save(model_q2.state_dict(),"best_sac_q2.pt")  
        #         torch.save(model_q2_target.state_dict(),"best_sac_q2_target.pt")  
...
def get_args():
    ...
    parser.add_argument("--saved_path", type=str, default="trained_models/sac")
    ...
    parser.add_argument("--burn_in", type=int, default="120000")
