...
class CustomReward(gym.RewardWrapper):
    ...
    def step(self, action):
        ...
        if info["coins"]>self.curr_coin:
            reward += (info["coins"]-self.curr_coin)*1200
            if info["score"] > (info["coins"]-self.curr_coin)*200+self.curr_score:
                reward += ((info["score"] - self.curr_score)-(info["coins"]-self.curr_coin)*200)*6
            self.no_progress=0            
        elif (info["score"] - self.curr_score)>0:
            reward += (info["score"] - self.curr_score)*6
            self.no_progress=0
        ...
        if info["status"]=="small" and info["x_pos"]>260: 
            self.regret=True
        if info["coins"]<12 and info["x_pos"]>580:
            self.regret=True
        if info["status"]!="fireball" and info["x_pos"]>830:
            self.regret=True
        if info["coins"]<18 and info["x_pos"]>760:
            self.regret=True 
        if info["status"]=="tall" and info["coins"]==12:
    ...
    def step(self, action):
        ...
        if done or self.stuck>64 or info_1[1]>256 or info_1[2]:            
            ...
            if self.stuck>64:
                ...
                self.pos = 40
                self.pre_pos=40
                return obs, reward-200/800, True, (info_1[0],info_1[1],65,False), info
            elif info_1[1]>256:
                self.stuck=0
                self.pos = 40
                self.pre_pos=40
                return obs, reward-500/800, True, (info_1[0],info_1[1],self.stuck,False), info
            elif info_1[2]:
                self.stuck=0
                self.pos = 40
                self.pre_pos=40
                return obs, reward-400/800, True, (info_1[0],info_1[1],self.stuck,True), info
            else:
                self.stuck=0
                self.pos = 40
                self.pre_pos=40
                return obs, reward-100/800, done, (info_1[0],info_1[1],self.stuck,False), info
...
def train(opt,writer):
    ...
    model = PPO((envs.num_states,263),len(COMPLEX_MOVEMENT))
    ...
    curr_episode = 0
    # explore_rate = 0.2
    action_queues = [deque([-1/48]*64,maxlen=64) for _ in range(opt.num_processes)]
    x_pos_queues = [deque([40/12800]*65,maxlen=65) for _ in range(opt.num_processes)]
    y_pos_queues = [deque([163/1024]*65,maxlen=65) for _ in range(opt.num_processes)]
    left_x_pos_queues = [deque([40/1024]*65,maxlen=65) for _ in range(opt.num_processes)]
    state_info = torch.zeros(opt.num_processes,263)
    for w1 in range(opt.num_processes):
        state_info[w1][0:64]=torch.FloatTensor(list(action_queues[w1]))
        state_info[w1][64:128]=torch.FloatTensor(list(x_pos_queues[w1])[0:64])
        state_info[w1][128:192]=torch.FloatTensor(list(y_pos_queues[w1])[0:64])
        state_info[w1][192:256]=torch.FloatTensor(list(left_x_pos_queues[w1])[0:64])
    for w1 in range(opt.num_processes):
        state_info[w1][256]=40/12800
        state_info[w1][257]=163/1024
        state_info[w1][258]=40/12800
        state_info[w1][259]=0/200
        state_info[w1][260]=0/200
        state_info[w1][261]=0 
        state_info[w1][262]=0/50 
    if torch.cuda.is_available():
    ...
    for iter in range(opt.num_global_steps):
        ...
        for local_steps in range(opt.num_local_steps):  
            ...
            # print(done)
            for w1 in range(opt.num_processes):
                # print(f"At process: {w1} at pos {info[w1]['x_pos']}")
                if done[w1] == True:
                    action_queues[w1] = deque([-1/48]*64,maxlen=64) 
                    x_pos_queues[w1] = deque([40/12800]*65,maxlen=65) 
                    y_pos_queues[w1] = deque([163/1024]*65,maxlen=65)                           
                    left_x_pos_queues[w1] = deque([40/1024]*65,maxlen=65)  
                    state_info[w1][0:64]=torch.FloatTensor(list(action_queues[w1]))
                    state_info[w1][64:128]=torch.FloatTensor(list(x_pos_queues[w1])[0:64])
                    state_info[w1][128:192]=torch.FloatTensor(list(y_pos_queues[w1])[0:64])
                    state_info[w1][192:256]=torch.FloatTensor(list(left_x_pos_queues[w1])[0:64])
                    ...
                    action_queues[w1].append(action[w1]/48)                  
                    x_pos_queues[w1].append(info[w1]["x_pos"]/12800)       
                    y_pos_queues[w1].append(info[w1]["y_pos"]/1024)       
                    left_x_pos_queues[w1].append(info[w1]["left_x_pos"]/1024) 
                    state_info[w1][0:64]=torch.FloatTensor(list(action_queues[w1]))
                    state_info[w1][64:128]=torch.FloatTensor(list(x_pos_queues[w1])[0:64])
                    state_info[w1][128:192]=torch.FloatTensor(list(y_pos_queues[w1])[0:64])
                    state_info[w1][192:256]=torch.FloatTensor(list(left_x_pos_queues[w1])[0:64])
                    state_info[w1][256]=info[w1]["x_pos"]/12800
                    state_info[w1][257]=info[w1]["y_pos"]/1024
                    state_info[w1][258]=info_1[w1][0]/12800
                    state_info[w1][259]=info_1[w1][1]/200
                    state_info[w1][260]=info_1[w1][2]/200
                    if info[w1]["status"]=="small":
                        state_info[w1][261]=0        
                    elif info[w1]["status"]=="tall":
                        state_info[w1][261]=0.1 
                    else:
                        state_info[w1][261]=0.2
                    state_info[w1][262]=info[w1]["coins"]/50
        ...
        advantages = advantages_ext.clone().detach()
        # else:
        ...
        r_state_info = torch.zeros(1,263)
        ...
        r_state_info[0][0:64]=torch.FloatTensor(list(action_queue))
        r_state_info[0][64:128]=torch.FloatTensor(list(x_pos_queue)[0:64])
        r_state_info[0][128:192]=torch.FloatTensor(list(y_pos_queue)[0:64])
        r_state_info[0][192:256]=torch.FloatTensor(list(l_x_pos_queue)[0:64])
        r_state_info[0][256]=40/12800
        r_state_info[0][257]=163/1024
        r_state_info[0][258]=40/12800
        r_state_info[0][259]=0/200
        r_state_info[0][260]=0/200
        r_state_info[0][261]=0
        r_state_info[0][262]=0/50
        time_spent=0
        stuck=0
        ...
        while True:
            ...
            r_state_info[0][128:192]=torch.FloatTensor(list(y_pos_queue)[0:64])
            r_state_info[0][192:256]=torch.FloatTensor(list(l_x_pos_queue)[0:64])
            r_state_info[0][256]=x_curr/12800
            r_state_info[0][257]=info["y_pos"]/1024
            r_state_info[0][258]=info_1[0]/12800
            r_state_info[0][259]=info_1[1]/200
            r_state_info[0][260]=info_1[2]/200
            if info["status"]=="small":
                r_state_info[0][261]=0        
            elif info["status"]=="tall":
                r_state_info[0][261]=0.1
            else:
                r_state_info[0][261]=0.2
            r_state_info[0][262]=info["coins"]/50
...
def get_args():
    ...
    parser.add_argument('--beta2', type=float, default=0.0011454*2, help='entropy coefficient')
    ...
    parser.add_argument("--num_local_steps", type=int, default=768)
