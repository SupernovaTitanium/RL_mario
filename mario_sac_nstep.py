...
class CustomReward(gym.RewardWrapper):
    ...
    def step(self, action):
        ...
        if info["coins"]>self.curr_coin:
            reward += (info["coins"]-self.curr_coin)*60*(1+info["x_pos"]/2400)*(1+info["y_pos"]/150)
            if info["score"] > (info["coins"]-self.curr_coin)*200+self.curr_score:
                reward += ((info["score"] - self.curr_score)-(info["coins"]-self.curr_coin)*200)*6*(1+info["x_pos"]/4800)/5
            self.no_progress=0            
        elif (info["score"] - self.curr_score)>0:
            reward += (info["score"] - self.curr_score)*6*(1+info["x_pos"]/4800)/5
            self.no_progress=0
    ...
    def step(self, action):
        ...
        if done or self.stuck>64 or info_1[1]>128 or info_1[2]:            
            ...
            if self.stuck>64:
                ...
                # self.y_pos = 79
                # self.y_pre_pos=79
                return obs, reward-10/200, True, (info_1[0],info_1[1],65,False), info
            elif info_1[1]>128:
                self.stuck=0
                self.x_pos = 40
                self.x_pre_pos=40
                # self.y_pos = 79
                # self.y_pre_pos=79
                return obs, reward-10/200, True, (info_1[0],info_1[1],self.stuck,False), info
            elif info_1[2]:
                self.stuck=0
                self.x_pos = 40
                self.x_pre_pos=40
                # self.y_pos = 79
                # self.y_pre_pos=79
                return obs, reward-10/200, True, (info_1[0],info_1[1],self.stuck,True), info
            else:
                self.stuck=0
                self.x_pos = 40
                self.x_pre_pos=40
                # self.y_pos = 79
                # self.y_pre_pos=79
                return obs, reward-5/200, done, (info_1[0],info_1[1],self.stuck,False), info
...
def train(opt,writer):
    ...
    # model_actor = torch.compile(model_actor)
    # model_q1 = torch.compile(model_q1)
    # model_q1_target = torch.compile(model_q1_target)
    # model_q2 = torch.compile(model_q2)
    # model_q2_target = torch.compile(model_q2_target)
    ...
    for iter in range(opt.num_global_steps):
        ...
        while True:
            if steps>opt.burn_in:
                ...
                action =torch.randint(0,len(COMPLEX_MOVEMENT),(1,))
        ...
        if steps>opt.burn_in:
            ...
            if curr_episode%opt.update_freq==0:
                ...
                writer.add_scalar('q1',mean_q1, curr_episode)
                writer.add_scalar('q2',mean_q2, curr_episode)
        ...
        if curr_episode % opt.update_freq == 0: 
            ...
            if temp_reward_sum>=max_reward:
                ...
                torch.save(model_actor.state_dict(),"best_sac_actor_nstep.pt")  
                torch.save(model_q1.state_dict(),"best_sac_q1_nstep.pt")  
                torch.save(model_q1_target.state_dict(),"best_sac_q1_target_nstep.pt")  
                torch.save(model_q2.state_dict(),"best_sac_q2_nstep.pt")  
                torch.save(model_q2_target.state_dict(),"best_sac_q2_target_nestep.pt")  
...
def get_args():
    ...
    parser.add_argument("--saved_path", type=str, default="trained_models/sac_n")
