...
env = gym_super_mario_bros.make("SuperMarioBros-1-1-v0", render_mode='human', apply_api_compatibility=True,render_mode='human')
...
mario_new = Mario(state_dim=(4, 84, 84), action_dim=env.action_space.n, save_dir="temp")
...
for try_iter in range(2):
    ...
    while True:
            ...
            reward_sum+=reward
            env.render()
            print(torch.equal(torch.from_numpy(np.array(next_state[0])),torch.from_numpy(np.array(state[0]))))
            # Update state
            state = next_state
            ...
            if done or info["flag_get"]:
                break
            print(f"Intermediate reward at step {step}: {reward}")
            step+=1
    print(f"Reward: {reward_sum} Steps: {step-1}")
